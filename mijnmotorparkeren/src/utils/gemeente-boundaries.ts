...
export class BoundaryCache {
  ...
  set(sourceUrl: string, data: GeoJSON.FeatureCollection): void {
    ...
    try {
      ...
      localStorage.setItem(`${this.CACHE_KEY}-${btoa(sourceUrl)}`, JSON.stringify(cacheEntry))
    } catch (error) {
...
export function matchGemeenteWithBoundary(
  ...
  if (boundaryName) {
    const normalizedBoundaryName = boundaryName
      .toLowerCase()
      .replace(/gemeente\s+/i, '')
      .trim()
    const normalizedGemeenteName = gemeente.name
      .toLowerCase()
      .replace(/gemeente\s+/i, '')
      .trim()
 ...
 */
export async function getGemeenteBoundary(gemeenteId: string): Promise<GeoJSON.Polygon | null> {
  try {
